...
plot(Bone1[[5]] , Bone1[[3]] ,  xlim = c(11000,0), ylim = c(-25,-8),
    main =  NULL,  pch=19,
     col = coll[1], xlab =  "Years Before Present",  
     ylab = expression(delta^13 * "C% VPDB"  ))
points(Bone2[[5]], Bone2[[3]], col = coll[2],  pch=19)
points(Bone3[[5]], Bone3[[3]], col =  coll[3],  pch=19)
points(Bone4[[5]], Bone4[[3]], col =  coll[4],  pch=19)
...
for (i in 1:nrow(data)) {
  ...
  print(temp)
  draw.circle(as.numeric(data[[5]][i]),as.numeric(data[[3]][i]),radius=300/1.2, col=coll[indexx])
  
floating.pie(as.numeric(data[[5]][i]), as.numeric(data[[3]][i]), temp, 
...
lines(c(-100000,1000000), c(-18,-18), col = "gray", lwd = 3, lty = 2)
