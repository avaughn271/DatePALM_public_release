...
plot(Bone1[[3]] , Bone1[[4]] , xlim = c(-24,-10), 
     ylim = c(7,20), main =  NULL,  pch=19,
     col = coll[1], ylab =  expression(delta^15 * "N% AIR"  ),  
     xlab = expression(delta^13 * "C% VPDB"  ))
points(Bone2[[3]], Bone2[[4]], col = coll[2],  pch=19)
points(Bone3[[3]], Bone3[[4]], col =  coll[3],  pch=19)
points(Bone4[[3]], Bone4[[4]], col =  coll[4],  pch=19)
...
for (i in 1:nrow(data)) {
  ...
  print(temp)
  draw.circle(as.numeric(data[[3]][i]),as.numeric(data[[4]][i]),radius=0.3, col=coll[indexx])
  
floating.pie(as.numeric(data[[3]][i]), as.numeric(data[[4]][i]), temp, 
             radius = 0.2, col = coll)
...
lines(c(-18,-18), c(-100000,1000000),  col = "gray", lwd = 3, lty = 2)
